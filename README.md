...
SpellMaster is an Airbag-compatible script specifically built to be expandable and to allow DMs to add homebrew content.  The SRD spells are all provided.  To add additional spells, make use of the Export feature (or manually edit the SRD.js file).
...
## Setup
1. Click Install below, as you would with any other spell.
...
5. In the chat menu, a simple menu will appear.  Press **[Create Spellbook]**.  You will be prompted for some basic information.  Fill them out.
...
- **DC**: You can specify the DC or the ability score to use for casting.
- **Upcast Cost**: Set the cost for upcasting the enchantment, presuming it can be upcast.  Click `Upcast Cost` to set if it can be upcast and the number itself to set the upcast cost per level.
...
- **[_] Azdregath's Opulent Ooze (R) - [-]**: The line with the spell's name on it has several features.
    - **[_]**: Clickin this empty checkbox will fill it with an X to mark it as prepared.  Cantrips are always checked.
...
- **Manually Prepared**: If you have a spell that is always prepared, you can lock it as prepared here as automatically prepared.  This can be useful for classes with permanently-known spells or spells with lists of auto-prepared lists.  A permanently-prepared spell is marked with a **[O]** in its preparedness checkbox.
...
- **Ability**: Clicking on this will allow you to set the spellcasting ability of the spell.  If you select **Manual DC**, you will be presented with the ability to set that value manually as well.  Note that any spell attack will have modifier 8 lower than the DC.
...
1. Create a **NEW** character sheet (the process will overwrite the character sheet).  I'll call it **House Spells**.  Navigate to its spell page.  This is the last time you'll use this page in SpellMaster.
...
8. Navigate to your campaign's API page and create a custom script.  Call it something like `SpellList.js` and paste in the **Bio & Info** code.
...
Your players will thereafter be able to add new spells to their character sheets.

## Performance
SpellMaster was largely born out of a dissatisfaction with the performance of existing tools for this reason...
- The master spell list is stored in an API script and the exports are handled through a character sheet, which have a significantly higher cap than the game state.  SpellMaster's data, were it to be stored in state would eat about a third of it.
- Expensive operations are cached, so only part of the page is regenerated at a time.
...
- **Resolution**: To fix this, I will need to upgrade chat integration.  A solution to this may come at the same time as regen rolling for items is upgraded.
- **Roadmap**: This is a high priority for me and will likely be addressed in some capacity in the next few versions.  Nevertheless, short of making a full-blown custom parser, there will **always** be something printed to chat.  Admittedly, I could obfuscate it in some capacity, so rolls are still effectively private.
## Zero-Zero
...
- **Resolution**: I'll need to build my own roll template or find another mechanism.  For instance, if I were to integrate with PowerCards, that would resolve the issue, though I would need assistance from their developers.
...
- **Improved NPC Support**: Currently, SpellMaster only supports PC characters.
...
- **Slotless Casting**: I would like to see some sort of damage ticking system that rerolls the description sheet, though I've yet to come up with a good solution.
